...
(function () {
  ...
  // ===== BEGIN: Testimonials (Sheet7: Nama | MediaURL) — v2.1 (momentum + a11y + GPU) =====
...
function pp_makeNodes(list) {
  ...
  list.forEach(({ name, url }) => {
    ...
    li.innerHTML = '<figure class="testi-fig">\
      <img src="' + url + '" alt="Testimoni ' + name.replace(/"/g,'&quot;') + '" loading="lazy" referrerpolicy="no-referrer">\
    </figure>\
...
async function loadTestimonials() {
  ...
  try {
    ...
    const rows = pp_csvParse(await res.text());
    ...
    // ----- Motion engine (auto + manual drag + momentum) -----
    let pos = 0;                         // translateX (px)
    let speed = 85;                      // px/s autoplay
    ...
    let vel = 0, lastX = 0, lastMoveTs = 0; // momentum
    let pausedHover = false, pausedUser = false, pausedHidden = false;
    let inertia = false;
    ...
    function step(ts) {
      ...
      const paused = pausedHover || pausedUser || pausedHidden || reduceMotion;
      if (!dragging && !paused && !inertia) {
        pos -= speed * dt;
        applyTransform();
      }
    ...
    // Hover pause (desktop)
    marquee.addEventListener('mouseenter', () => { pausedHover = true; });
    marquee.addEventListener('mouseleave', () => { pausedHover = false; });

    // Pause when offscreen + tab hidden
    const io = 'IntersectionObserver' in window ? new IntersectionObserver((entries)=>{
      pausedHidden = entries[0] ? !entries[0].isIntersecting : false;
    }, { root: null, threshold: 0 }) : null;
    io && io.observe(section);
    document.addEventListener('visibilitychange', ()=>{
      pausedHidden = (document.visibilityState !== 'visible');
    });

    // Touch/mouse drag with momentum
    function onDown(e) {
      dragging = true; inertia = false; track.classList.add('dragging');
      startX = lastX = (e.touches ? e.touches[0].clientX : e.clientX);
      startPos = pos; vel = 0; lastMoveTs = performance.now();
      e.preventDefault();
    }
    function onMove(e) {
      if (!dragging) return;
    ...
    function onUp() {
      ...
      // inertia glide
      let v = vel; inertia = Math.abs(v) > 40;
      if (!inertia) return;
      const decay = 0.0018;         // lower = longer glide
      const min = 6;                // stop threshold (px/s)
      let ts0 = performance.now();
      (function glide(ts){
        const dt = (ts - ts0) / 1000; ts0 = ts;
        const sign = Math.sign(v);
        v -= sign * Math.max(0, Math.abs(v) * decay * (dt * 1000));
        if (Math.abs(v) <= min) { inertia = false; return; }
        pos += v * dt;
        applyTransform();
    ...
    // iOS fallback
    marquee.addEventListener('touchstart', onDown, { passive: false });
    window.addEventListener('touchmove', onMove, { passive: false });
    window.addEventListener('touchend', onUp, { passive: true });

    // Accessible Pause/Play button (visually hidden until focus)
    const toggle = document.getElementById('testiToggle');
    if (toggle) {
      toggle.addEventListener('click', () => {
        pausedUser = !pausedUser;
        toggle.setAttribute('aria-pressed', String(pausedUser));
        toggle.textContent = pausedUser ? 'Play' : 'Pause';
      });
    }

    // Keyboard navigation (left/right nudge)
    window.addEventListener('keydown', (e) => {
      if (e.key === 'ArrowRight') { pos -= 240; applyTransform(); }
      if (e.key === 'ArrowLeft')  { pos += 240; applyTransform(); }
    });
