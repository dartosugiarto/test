(function (global) {
  ...
  function renderMenu(container, items, onRoute, closeAll){
    ...
    items.forEach(item => {
      ...
      if (item.type === 'route') btn.addEventListener('click', () => { onRoute?.(item.value); closeAll(); });
      else btn.addEventListener('click', () => { window.open(item.href, '_blank', 'noopener'); closeAll(); });
