...
(function () {
  ...
  // --- SEMUA FUNGSI UTILITAS, PARSING, DAN RENDER TETAP SAMA ---
  // ... (copy semua fungsi dari kode sebelumnya mulai dari formatToIdr hingga initializeAccounts) ...
  ...
  function initializeApp() {
    ...
    // =================================================================
    // === PERBAIKAN UTAMA: MEMASTIKAN SEMUA DROPDOWN MENDAPAT EVENT ===
    // =================================================================
    // Daripada memasang listener satu per satu di fungsi terpisah (yang mungkin gagal),
    // kita pasang semua di sini saat aplikasi pertama kali dimuat.
    ...
    document.addEventListener('click', e => {
      ...
      // Periksa apakah klik BUKAN pada tombol dropdown itu sendiri sebelum menutupnya.
      // Ini mencegah konflik buka/tutup di saat yang sama.
  ...
  // Masukkan kembali semua fungsi yang hilang dari versi sebelumnya
...
(function() {
  function installRobustSelect(optionsEl) {
    if (!optionsEl) return;
    ...
    function onDown(e) {
      ...
      moved = false;
      downTarget = (e.target && e.target.closest) ? e.target.closest('.custom-select-option') : null;
    ...
    function onMove(e) {
      ...
      if (!p) return;
      if (Math.abs(p.clientX - startX) > 10 || Math.abs(p.clientY - startY) > 10) moved = true;
    }
    function onUp(e) {
      window.removeEventListener('pointermove', onMove);
      window.removeEventListener('touchmove', onMove);
      if (downTarget && !moved) {
        try { downTarget.click(); } catch(_) {}
      }
      downTarget = null;
    }
    optionsEl.addEventListener('pointerdown', onDown, {passive:false});
    optionsEl.addEventListener('touchstart', onDown, {passive:false});
  }
  try {
    installRobustSelect(document.getElementById('layananCustomSelectOptions'));
    installRobustSelect(document.getElementById('preorderCustomSelectOptions'));
    installRobustSelect(document.getElementById('accountCustomSelectOptions'));
  } catch(_) {}
})();
