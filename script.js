 ...
 * @version 5.0.0 (Production-ready, implemented Smart Gesture System for professional-grade UI interaction)
 */

(function () {
  'use strict';

  // ... (Bagian config, state, dan elements tetap sama persis) ...
  const config = {
  ...
  // --- DATA PARSING (Tidak ada perubahan) ---
  ...
  // --- CATALOG & OTHER MODULES (Tidak ada perubahan signifikan, hanya memastikan konsistensi) ---
  ...
  // --- GESTURE DETECTION SYSTEM (Teknologi Baru) ---
  function initSmartGestureSystem(callback) {
    let startX = 0;
    let startY = 0;
    let isScrolling = false;
    const scrollThreshold = 10; // Jarak minimal (pixel) untuk dianggap scroll
    ...
    document.addEventListener('pointerup', (e) => {
      if (!isScrolling) {
        // Ini adalah TAP murni, jalankan callback
        callback(e);
      }
    }, { capture: true }); // Gunakan capture phase untuk menangani event lebih dulu
  }
  ...
  function initializeApp() {
    ...
    // Inisialisasi Sistem Deteksi Gestur Cerdas
    initSmartGestureSystem((e) => {
      const target = e.target;
      customSelects.forEach(select => {
        // Tutup dropdown hanya jika tap terjadi di luar area dropdown
